...
@ApiResponses(value = { @ApiResponse(code = 200, message = "Successfully retrieved all records"),
...
public class BooksController {
	...
	public ResponseEntity<BookAuthorResponse> addBookdetails(@RequestBody BooksRequest booksRequest) {
		...
		if ((null != booksRequest.getBooktitle() && !booksRequest.getBooktitle().isEmpty())
			...
			if (status.equalsIgnoreCase("success")) {
				return new ResponseEntity<BookAuthorResponse>(
						new BookAuthorResponse(true, "Book Record Inserted", booksRequest), HttpStatus.CREATED);
			}
		} else {
			return new ResponseEntity<BookAuthorResponse>(
					new BookAuthorResponse(false, "Invalid Input Parameter", null), HttpStatus.BAD_REQUEST);
		}
		return new ResponseEntity<BookAuthorResponse>(
				new BookAuthorResponse(false, "Failed To Insert Book Record", null), HttpStatus.INTERNAL_SERVER_ERROR);
	}
	...
	public ResponseEntity<BookAuthorResponse> getAllBookRecord() {
		...
		if (null != booksList && !booksList.isEmpty()) {
			return new ResponseEntity<BookAuthorResponse>(
					new BookAuthorResponse(true, "Books Record Fetched", booksList), HttpStatus.OK);
		} else {
			return new ResponseEntity<BookAuthorResponse>(new BookAuthorResponse(false, "Books Record Not Found", null),
	...
	public ResponseEntity<BookAuthorResponse> getSingleBookRecord(@PathVariable(value = "bookid") String bookid) {
		...
		if (null != booksList && !booksList.isEmpty()) {
			return new ResponseEntity<BookAuthorResponse>(
					new BookAuthorResponse(true, "Book Record Fetched", booksList), HttpStatus.OK);
		} else {
			return new ResponseEntity<BookAuthorResponse>(new BookAuthorResponse(false, "Book Record Not Found", null),
					HttpStatus.NOT_FOUND);
		}
	}
	...
	public ResponseEntity<BookAuthorResponse> updateSingleBookRecord(@PathVariable(value = "bookid") String bookid,
		...
		if ((null != booksRequest.getBooktitle() && !booksRequest.getBooktitle().isEmpty())
			...
			if (status.equalsIgnoreCase("success")) {
				return new ResponseEntity<BookAuthorResponse>(
						new BookAuthorResponse(true, "Book Record Updated", booksRequest), HttpStatus.CREATED);
			}
		} else {
			return new ResponseEntity<BookAuthorResponse>(
					new BookAuthorResponse(false, "Invalid Input Parameter", null), HttpStatus.BAD_REQUEST);
		...
		return new ResponseEntity<BookAuthorResponse>(
				new BookAuthorResponse(false, "Failed To Update Book Record", null), HttpStatus.INTERNAL_SERVER_ERROR);
	}
	...
	public ResponseEntity<BookAuthorResponse> deleteBookRecordByid(@PathVariable(value = "bookid") String bookid) {
		...
		if (null != status && status.equalsIgnoreCase("success")) {
			return new ResponseEntity<BookAuthorResponse>(new BookAuthorResponse(true, "Book Record Deleted", status),
					HttpStatus.OK);
		} else {
			return new ResponseEntity<BookAuthorResponse>(
					new BookAuthorResponse(true, "Failed To Delete Bookrecord", status), HttpStatus.EXPECTATION_FAILED);
		}
	}
}
